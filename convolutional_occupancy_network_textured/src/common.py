...
def normalize_3d_coordinate(p, padding=0.1):
    ...
    # p range (-0.55, 0.55)
    p_nor = p / (1 + padding + 10e-4) # (-0.5, 0.5) # torch.size([16, 2048/3000, 3])
...
def normalize_color_value(col_nor):
    ''' Normalize coordinate to [0, 1] for unit cube experiments.
        Corresponds to our 3D model

    Args:
        p (tensor): color rgb value
    '''

    if col_nor.max() >= 1:
        col_nor[col_nor >= 1] = 1 - 10e-4
    if col_nor.min() < 0:
        col_nor[col_nor < 0] = 0.0
    return col_nor
...
def coordinate2index(x, reso, coord_type='2d'):
    ...
    x = (x * reso).long() # torch.Size([64, 32768, 2]), in range (0, 64)
    if coord_type == '2d': # plane
        index = x[:, :, 0] + reso * x[:, :, 1] # torch.Size([64, 32768])
    elif coord_type == '3d': # grid
        index = x[:, :, 0] + reso * (x[:, :, 1] + reso * x[:, :, 2])
    index = index[:, None, :] # torch.Size([64, 1, 32768])
    return index
