...
class VoxelsField(Field):
    ...
    '''
    def __init__(self, file_name, file_name_col, transform=None):
        self.file_name = file_name
    ...
    def load(self, model_path, idx, category):
        ...
        file_path = os.path.join(model_path, self.file_name)
        file_path_col = os.path.join(model_path, self.file_name_col)
        ...
        voxels_color_dict = np.load(file_path_col)
        voxels_color = voxels_color_dict['voxels_color'] # (32,32,32,3)
        voxels_color = voxels_color.astype(np.float32)

        data = {
            None: voxels,
            'voxels_color': voxels_color,
        }
