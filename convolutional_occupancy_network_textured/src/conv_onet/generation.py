...
class Generator3D(object):
    ...
    def generate_mesh(self, data, it, num, return_stats=True):
        ...
        inputs = data.get('inputs', torch.empty(1, 0)).to(device) # torch.Size([1, 32, 32, 32])
        inputs_color = data.get('inputs.voxels_color', torch.empty(1, 0)).to(device) # torch.Size([1, 32, 32, 32, 3])
        kwargs = {}
        ...
        if self.vol_bound is not None:
            ...
            # print("Encode inputs")
            inputs = add_key(inputs, data.get('inputs.ind'), 'points', 'index', device=device) # torch.Size([1, 32, 32, 32]) [0,1]

            t0 = time.time()
            with torch.no_grad():
                # c = self.model.encode_inputs(inputs) # dict_keys(['xz', 'xy', 'yz']), torch.Size([1, 32, 64, 64])
                c = self.model.main_encode_inputs(inputs, inputs_color) # ['xz', 'xy', 'yz', 'xz_r', 'xz_g', 'xz_b', ...]
        stats_dict['time (encode inputs)'] = time.time() - t0
        
        mesh, vertices, triangles = self.generate_from_latent(c, stats_dict=stats_dict, **kwargs)

        if return_stats:
            return mesh, vertices, triangles, stats_dict
        else:
    ...
    def generate_from_latent(self, c=None, stats_dict={}, **kwargs):
        ...
        if self.upsampling_steps == 0:
            ...
            # print("MISE algorithm")
            mesh_extractor = MISE(self.resolution0, self.upsampling_steps, threshold) # reso: 32, 2
            points = mesh_extractor.query() # (35937, 3) [0,0,0], [0,0,4], ... [128,128,128]

            pointsc = points / mesh_extractor.resolution * 1.1 - 0.55 # [0,0,0], ..., [1,1,1] / 128
            pointsc = torch.FloatTensor(pointsc).to(self.device)
            valuesc = self.eval_points(pointsc, c, **kwargs).cpu().numpy() # (35937, 3)
            valuesc = valuesc.astype(np.float64)
            # print("query points", pointsc.shape)
            # print("query points", valuesc.shape)
            # print("points shape", points.shape)


            while points.shape[0] != 0:
                # Query points
                pointsf = points / mesh_extractor.resolution # [0,0,0], ..., [1,1,1] / 128
                # Normalize to bounding box
                pointsf = box_size * (pointsf - 0.5)
                pointsf = torch.FloatTensor(pointsf).to(self.device)
                # Evaluate model and update
                values = self.eval_points(pointsf, c, **kwargs).cpu().numpy() # (35937, 3)
                values = values.astype(np.float64)
                # mesh_extractor.update(points, values)
                mesh_extractor.update(points, values[:, 0])
                points = mesh_extractor.query() # (36157, 3) -> ... -> (53, 3) -> (0, 3)
            value_grid = mesh_extractor.to_dense() # (129, 129, 129)

        # Extract mesh
        stats_dict['time (eval points)'] = time.time() - t0
        mesh, vertices, triangles  = self.extract_mesh(value_grid, c, valuesc, pointsc, stats_dict=stats_dict)
        return mesh, vertices, triangles
    ...
    def eval_points(self, p, c=None, vol_bound=None, **kwargs):
        ...
        Args:
            ...
            c (tensor): encoded feature volumes
        '''
        p_split = torch.split(p, self.points_batch_size) # points: torch.Size([35937, 3])
        occ_hats = []
        for pi in p_split:
            if self.input_type == 'pointcloud_crop':
                ...
                # print("Evaluate mesh")
                pi = pi.unsqueeze(0).to(self.device) # pi: torch.Size([1, 35937, 3]) -> torch.Size([1, 35937, 3])
                with torch.no_grad():
                    occ_hat = self.model.main_decode(pi, c, **kwargs).logits # torch.Size([1, 35937, 4])
                    # occ_hat = self.model.decode(pi, c, **kwargs).logits # torch.Size([1, 35937])
                occ_hats.append(occ_hat.squeeze(0).detach().cpu())
        occ_hat = torch.cat(occ_hats, dim=0) # torch.Size([35937, 3])
        return occ_hat

    def extract_mesh(self, occ_hat, c=None, values=None, pointsf=None, stats_dict=dict()):
        ...
        occ_hat_padded = np.pad(
            occ_hat, 1, 'constant', constant_values=-1e6) # (131, 131, 131)
        #print("occ hat padded", occ_hat.shape)
        vertices, triangles = libmcubes.marching_cubes(
            occ_hat_padded, threshold) # (142972, 3) (286292, 3)
        ...
        print("pointsf max", pointsf.max())
        print("pointsf min", pointsf.min())

        print("pointsf shape", pointsf.shape)
        print("colors shape", values.shape)
        print("vertices", vertices.shape)

        print("values max", values[:, [1, 2, 3]].max())
        print("values min", values[:, [1, 2, 3]].min())
        '''
        colors = self.estimate_color(vertices, pointsf, values) # pointsf: torch.Size([35937, 3]), values: (35937, 4), vertices (99846, 3)
        colors[colors > 255] = 255 - 1e-6
        colors[colors < 0] = 0
        ...
        vertices = np.concatenate((vertices, colors_np), axis=1)

        # print("path", os.path.join(os.getcwd(), "out/voxels/shapenet_grid32/vis/mesh_{}_{}.obj".format(self.it, self.num)))
        # mc.export_obj(vertices, triangles, os.path.join(os.getcwd(), "out/voxels/shapenet_grid32/vis/mesh_{}_{}.obj".format(self.it, self.num)))

        return mesh, vertices, triangles

    def estimate_color(self, v, p, c):
        color = torch.empty(v.shape[0], v.shape[1])
        for i in range(len(v)):
            idx = ((p.cpu() - v[i, :]) ** 2).sum(1).argmin()
            color[i, :] = torch.from_numpy(c[idx, [1, 2, 3]]).to(color)
        return color
