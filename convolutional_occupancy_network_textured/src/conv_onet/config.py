...
def get_model(cfg, device=None, dataset=None, **kwargs):
    ...
    main_encoder = cfg['model']['main_encoder']
    main_decoder = cfg['model']['main_decoder']
    main_decoder_kwargs = cfg['model']['main_decoder_kwargs']
    main_encoder_kwargs = cfg['model']['main_encoder_kwargs']
    ...
    main_decoder = models.decoder_dict[main_decoder](
        dim=dim, c_dim=c_dim, padding=padding,
        **main_decoder_kwargs
    )

    main_encoder = encoder_dict[main_encoder](
        dim=dim, c_dim=c_dim, padding=padding,
        **main_encoder_kwargs
    )

    model = models.ConvolutionalOccupancyNetwork(
        main_decoder, main_encoder, device=device
    )
...
def get_data_fields(mode, cfg):
    ...
    input_type = cfg['data']['input_type'] # voxels

    test = cfg['data']['points_file']
    ...
    if mode in ('val', 'test'):
        ...
        if voxels_file is not None:
            voxels_color_file = cfg['data']['voxels_color_file']
            fields['voxels'] = data.VoxelsField(voxels_file, voxels_color_file)
            #fields['voxels'] = data.VoxelsField(voxels_file)

    return fields # points, points_iou, voxels
