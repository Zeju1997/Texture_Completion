...
class Fusion:
    ...
    def __init__(self, input_folder, output_folder):
        ...
        parser = self.get_parser(input_folder, output_folder)
    ...
    def get_parser(self, input_folder, output_folder):
        """
        Get parser of tool.
        ...
        parser.add_argument('--mode', type=str, help='Operation mode: render, fuse or sample.')
        input_group = parser.add_mutually_exclusive_group(required=False)
        input_group.add_argument('--in_dir', default=input_folder, type=str, help='Path to input directory.')
        input_group.add_argument('--in_file', type=str, help='Path to input directory.')
        parser.add_argument('--out_dir', type=str, default=output_folder, help='Path to output directory; files within are overwritten!')
    ...
    def render(self, mesh, Rs):
        ...
        mesh_color = mesh.vertices[:, [3, 4, 5]]
        filename = os.path.join(self.options.out_dir, "vcolor.npy")
        #Path(file_folder).mkdir(parents=True, exist_ok=True)
        #np.save(filename, mesh_color.astype(np.float64))

        depthmaps = []
        for i in range(len(Rs)):
            mesh_pos = mesh.vertices[:, [0, 1, 2]]
            np_vertices = Rs[i].dot(mesh_pos.astype(np.float64).T)
...
if __name__ == '__main__':
    ...
    with open(data_file, 'r') as f:
        ...
        for sample in models_c:
            if sample != "":
                obj_folder = os.path.join(data_folder, "my_shapenet/03001627", sample)
                if sys.argv[1] == "--mode=render":
                    # render
                    input_folder = os.path.join(obj_folder, "1_scaled")
                    output_folder = os.path.join(obj_folder, "2_depth")
                    Path(input_folder).mkdir(parents=True, exist_ok=True)
                    Path(output_folder).mkdir(parents=True, exist_ok=True)
                    app = Fusion(input_folder, output_folder)
                    app.run()
                    temp = temp + 1
                    print(temp)
                    #if os.path.exists(input_folder) and os.path.isdir(input_folder):
                    #    shutil.rmtree(input_folder)
                if sys.argv[1] == "--mode=fuse":
                    # fusion
                    input_folder = os.path.join(obj_folder, "2_depth")
                    output_folder = os.path.join(obj_folder, "2_watertight")
                    Path(input_folder).mkdir(parents=True, exist_ok=True)
                    Path(output_folder).mkdir(parents=True, exist_ok=True)
                    app = Fusion(input_folder, output_folder)
                    app.run()
