...
class Scale:
    ...
    def get_parser(self, input_folder, output_folder):
        ...
        input_group.add_argument('--in_dir', type=str, default=input_folder, help='Path to input directory.')
        ...
        parser.add_argument('--overwrite', action='store_true', default=True, help='Overwrites existing files if true.')
        return parser
    ...
    with open(data_file, 'r') as f:
        ...
        for sample in models_c:
            if sample != "":
                ...
                input_folder = os.path.join(sample_folder, "obj")
                output_folder = os.path.join(sample_folder, "1_scaled")
                Path(input_folder).mkdir(parents=True, exist_ok=True)
                Path(output_folder).mkdir(parents=True, exist_ok=True)
                app = Scale(input_folder, output_folder)
                app.run()
