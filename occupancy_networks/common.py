...
def write_obj(file, vertices, faces):
    ...
    with open(file, 'w') as fp:
        for vertex in vertices:
            assert len(vertex) == 6, 'invalid vertex with %d dimensions found (%s)' % (len(vertex), file)
            fp.write('v' + ' ' + str(vertex[0]) + ' ' + str(vertex[1]) + ' ' + str(vertex[2]) + ' ' + str(vertex[3]) + ' ' + str(vertex[4]) + ' ' + str(vertex[5]) + '\n')
...
def read_obj(file):
    ...
    with open(file, 'r') as fp:
        ...
        for line in lines:
            ...
            if parts[0] == 'v':
                assert len(parts) == 7 or len(parts) == 4, \
                ...
                assert parts[4] != '', 'rgb value for r is empty (%s)' % file
                assert parts[5] != '', 'rgb value for g is empty (%s)' % file
                assert parts[6] != '', 'rbg value for b is empty (%s)' % file
...
class Mesh:
    ...
    def __init__(self, vertices = [[]], faces = [[]]):
        ...
        assert self.vertices.shape[1] == 6
